...
pencere.title("BAĞLANDI: "+ ip +" "+ str(port))

message = Text(pencere, width=40)
message.grid(row =0, column=0, padx=10, pady=10,)

mesaj_giris = Entry(pencere, width=50)
mesaj_giris.insert(0, "ADINIZ SOYADINIZ*")
...
def mesaj_gonder():
    ...
    message.insert(END, '\n' + 'SEN:'+ istemci_mesaji+'  TARİH: '+timestamp)
...
def dosya_gonder():
    ...
    with open(dosya_adı, 'rb') as file:
        for data in file:
            client.send(data)
    print(f"{dosya_adı} DOSYASI SORUNSUZ İLETİLDİ ")
...
btn_dosya_gonder = Button(pencere, text="BİR DOSYA GÖNDER", width=30, command=dosya_gonder)
